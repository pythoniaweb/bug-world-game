//var script = document.createElement('script');script.onload = function () {this.id = 'phazorJS'};script.src = 'http://api.boomhost.ml/phazor.js';document.head.appendChild(script);console.group("Phazor.js Load Master Started");console.log('Phazor.JS Installing');console.groupEnd("Phazor.js Load Master Started")
...
pjs.favicontext = function(text) { /* Phazor.JS text facicon */
...
pjs.favicon = function(durl) { /* Phazor.JS facicon */
  const link = document.createElement('link');
  const oldLinks = document.querySelectorAll('link[rel="shortcut icon"]');
  oldLinks.forEach(e => e.parentNode.removeChild(e));
  link.id = 'dynamic-favicon';
...
pjs.terror = function(text) { /* Phazor.JS throw new Error */
   throw new Error("Phazor.JS can only run in windows with a document.");
};  
pjs.ctable = function(data) { /* Phazor.JS throw new Error */
  console.table([data]);
};
pjs.cclear = function() { /* Phazor.JS throw new Error */
  console.clear();
};
pjs.cassert = function(error, data) { /* Phazor.JS throw new Error */
  console.assert(error, data)
};
pjs.sgroup = function(data) { /* Phazor.JS throw new Error */
  console.group(data);
};
pjs.egroup = function(data) { /* Phazor.JS throw new Error */
  console.groupEnd(data)
};
pjs.cstime = function() { /* Phazor.JS throw new Error */
  console.time()
};
pjs.cetime = function() { /* Phazor.JS throw new Error */
  console.timeEnd()
}
